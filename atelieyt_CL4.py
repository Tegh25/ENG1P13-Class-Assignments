...
def determine_update(device_info):
    ...
    if update_req == True:
        if(OSType == "iOS"):
            ...
            if(OSVersion >= 10.0):
                return "Version 2"
            elif(OSVersion >= 8.0 and OSVersion < 10.0):
...
def determine_device_update(devices):
    ...
    for i in devices:
        if update_required(i) == True:
            list = [i[0], determine_update(i)]
            update_needed.append(list)
...
# 
# 1. It was beneficial to create two separate functions because the code is much more organized and easy to read this way especially since I had to call those two functions inside a loop in the third function. If I did not create the update_required function, I'd have to add in the code from update_required into determine_update and determine_device_update and the code would start looking very cluttered.
# 
# 2. It is possible to use a while loop for the for loop, although I feel like it's more beneficial to use a for loop in this case because it allows us to iterate through the lists easier and better. A while loop would be more beneficial than a for loop in a situation where we don't know exactly how many times we will have to repeat a task. For example, if you wanted to repeat a piece of code until a user stopped it with a certain input, you would use a while loop.
# 
# ```
