...
# variables of gravity and distance defined
g = 9.81
d = float(input("Please enter the distance to our target zombie:"))

# generate angle and convert to radians
theta = random.random() * 45.0
theta_rads = math.radians(theta)

# use the variables above to calculate velocity
v = (g*d)/(math.sin(2*theta_rads))
v = math.sqrt(v)
...
# 
# 1.
# If the user enters a negative value, a value error would occur. This because in the equation for speed, a negative distance would make the value inside the square root negative. It is not possible to calculate the square root of a negative number.
# If the user enters a non-numeric character, a value error would occur. This is because the input is converted to a float immediately after being entered. A character that is not a number would not be able to convert.
# 
# 2.
# No it wouldn't. This is because the compiler checks over the whole code before running it. As long as g = 9.81 is defined somewhere in the scope of the calculation for V, the code will have the definition of g and the behaviour will not change.
# ```
