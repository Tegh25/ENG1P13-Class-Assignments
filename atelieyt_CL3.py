...
# This function takes takes the string from a specific index of the input list and extracts the final grade from it as a float
def extract_grade(x):
    list = x.split("-")
    grade = list[1].strip("%")
    return float(grade)
...
# This function takes the final grades of each student returned by the extract_grade function and calculates the mean value of all of them
def class_average(final_marks):
    num = 0
    for i in final_marks:
        num += extract_grade(i)
    return num / len(final_marks)
    
#********************************************
# Write your class_std_dev function below: (3 marks)
#********************************************

# This function calulates the standard derivation of all of the student's final grades using the grades returned from extract_grade and the average returned by class_average
def class_std_dev(final_marks):
    num = 0
    avg = class_average(final_marks)
    for i in final_marks:
        num += (extract_grade(i) - avg)**2
    return (num / len(final_marks))**0.5
...
def main():
    ...
    # final_exam_grades = ["83-100%","1001425-199.5%","1009980-118.2%","1001480-184.0%","400199444-187.9%"]
...
# 
# 1. As long as the correct format is followed, the only input that will cause the program to fail is if the inputted list was empty.
# 
# 2. The only modification would need to be implemented in the extract_grade function. The split method will use the parameter ":" instead of "-", and the strip method will be removed.
# 
# 3. No because to compute the standard deviation, you need to use the value of the average grade. You cannot obtain this value until after the for loop calculating the value for average grade is finished.
# 
# ```
